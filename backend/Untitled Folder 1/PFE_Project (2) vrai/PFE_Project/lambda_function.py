...
lambda_function.py
──────────────────
AWS Lambda handler — same logic as app.py but without Flask.

How it works:
  - AWS API Gateway sends HTTP requests to Lambda as events
  - lambda_handler() reads the event, calls the orchestrator,
    and returns an API Gateway-compatible response

Deploy steps (simple):
  1. Zip the whole project folder
  2. Upload to Lambda (Python 3.11 runtime)
  3. Set handler to: lambda_function.lambda_handler
  4. Add environment variables matching your .env file
  5. Attach an API Gateway trigger

Supported routes (via event["path"]):
  POST /ask    → main chat
...
# ── These modules are bundled with your Lambda deployment zip ─
