...
# French words that clearly indicate the user is writing in French
_FRENCH_WORDS = {
    "montre", "affiche", "donne", "quel", "quelle", "quels", "quelles",
    "combien", "comment", "pourquoi", "quand", "meilleur", "meilleurs",
    "total", "moyenne", "ventes", "chiffre", "produit", "produits",
    "région", "regions", "categorie", "catégorie", "tendance", "rapport",
    "graphique", "histogramme", "camembert", "classement", "top", "les",
    "des", "par", "sur", "est", "sont", "mes", "mon", "ma", "tu", "je",
    "bonjour", "salut", "merci", "aide", "génère", "genere", "fais",
    "montre-moi", "dis-moi", "qu'est", "c'est", "faire", "voir"
}

def detect_language(text: str) -> str:
    """
    Returns 'fr' if the text appears to be French, else 'en'.
    Simple word-matching — no external library needed.
    """
    words = set(text.lower().replace("'", " ").replace("-", " ").split())
    french_hits = words & _FRENCH_WORDS
    return "fr" if len(french_hits) >= 1 else "en"
...
def parse_question(question: str, df_meta: dict, memory: list, lang: str = "en") -> dict:
    """
    ...
    chart_keywords = [
        ...
        "classement", "visualise", "courbe", "diagramme"
    ]
    wants_chart = any(kw in question.lower() for kw in chart_keywords)

    # Language instruction appended to the prompt
    lang_instruction = (
        "⚠️ IMPORTANT: The user is writing in FRENCH. Your 'intent' field must be in French. "
        "The routing JSON stays in English (field names and values like 'analytics', 'top_n' etc. don't change)."
        if lang == "fr" else ""
    )
...
- trend/over time/monthly/tendance/mensuel → metric=trend, chart_type=line
- top/best/ranking/meilleur/classement    → metric=top_n, chart_type=bar
- distribution/histogram/histogramme      → metric=distribution, chart_type=hist
- pie/share/proportion/camembert          → chart_type=pie
- "time_period": monthly | yearly | daily | null
- rapport/report/summary/export           → type=report
"""
    try:
...
def ask_rag(question: str, lang: str = "en") -> dict:
    """
    Search the Bedrock Knowledge Base and generate an answer.
    Replies in French if lang='fr'.
    Returns {"text": "...", "chart": None}
    """
    # Prepend a language instruction so Bedrock answers in the right language
    lang_prefix = (
        "Tu es un assistant commercial. Réponds TOUJOURS en français. "
        if lang == "fr" else
        "You are a helpful sales assistant. Reply in English. "
    )
    augmented_question = lang_prefix + question
    try:
        ...
        if not answer:
            answer = (
                "Je n'ai pas trouvé de réponse dans la base de connaissances."
                if lang == "fr" else
